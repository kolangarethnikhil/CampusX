  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden">
      <div className="relative pt-14 px-5 pb-5 bg-gradient-radial overflow-hidden">
        ...
        <h1 className="relative text-[28px] font-semibold text-cx-text tracking-tight mb-1">Spaces</h1>
        <p className="relative text-cx-text-muted text-[11px] tracking-wide mb-5">Admin-controlled chat rooms for KJU</p>
      ...
      <div className="flex-1 overflow-y-auto px-5 pt-2 pb-3 bg-gradient-mesh">
        <div className="space-y-2.5">
          {filteredSpaces.map((space, i) => {
            ...
            return (
              ...
              >
                <div className="flex h-13 w-13 min-h-13 min-w-13 items-center justify-center rounded-2xl flex-shrink-0 overflow-hidden border border-white/[0.045]" style={{ background: `${space.accent}12` }}>
                  <AppIcon name={icon} size={43} />
                </div>
                <div className="flex-1 text-left min-w-0">
                  <div className="flex items-center gap-2 mb-0.5">
                    <span className="text-[14px] font-semibold text-cx-text tracking-[-0.02em]">{space.name}</span>
                    {active && <span className="w-1.5 h-1.5 rounded-full bg-cx-lime shadow-[0_0_7px_rgba(163,230,53,0.7)]" />}
                  </div>
                  <p className="text-cx-text-muted text-[12px] truncate">{space.description}</p>
                  <p className="text-[10px] text-cx-text-muted mt-1">{space.memberCount || 0} members · {displayCategory(space.category)}</p>
                </div>
                <div className="flex flex-col items-end gap-1">
                  <span className="text-cx-lime text-[10px] font-semibold">{active ? "Live" : "Open"}</span>
        ...
        <div className="mt-4 grid grid-cols-2 gap-3">
          <button onClick={handleRequestSpace} className="rounded-2xl glass-subtle interactive-glass p-4 text-left">
            <div className="w-9 h-9 rounded-xl bg-cx-purple/10 text-cx-purple-bright flex items-center justify-center mb-3">
              <UserPlus size={16} />
            </div>
            <p className="text-[12px] font-semibold text-cx-text">Request space</p>
            <p className="text-[10px] text-cx-text-muted mt-1 leading-relaxed">Ask admin to create a new room.</p>
          </button>
          <button onClick={handleRequestSpace} className="rounded-2xl glass-subtle interactive-glass p-4 text-left">
            <div className="w-9 h-9 rounded-xl bg-cx-lime/10 text-cx-lime flex items-center justify-center mb-3">
              <ShieldCheck size={16} />
            </div>
            <p className="text-[12px] font-semibold text-cx-text">Be moderator</p>
            <p className="text-[10px] text-cx-text-muted mt-1 leading-relaxed">Open a space and request role.</p>
          </button>
        </div>
        {status && <p className="text-[10px] text-cx-text-muted text-center mt-3">{status}</p>}
      </div>
    </div>
  );
}
