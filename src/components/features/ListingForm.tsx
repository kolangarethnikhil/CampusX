...
interface ListingFormData {
  title: string;
  roomType: "single" | "shared" | "1BHK" | "PG";
...
export default function ListingForm({
  ...
  const validateForm = () => {
    ...
    if (!formData.latitude || !formData.longitude) {
      return "Please select a campus location on the map.";
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!user || formData.latitude === null || formData.longitude === null) {
      return;
    }
    ...
    try {
      ...
      alert("Failed to create listing. Please check Firebase rules and try again.");
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="flex justify-between items-center mb-10 sticky top-0 bg-kjc-black/80 backdrop-blur-xl z-20 pb-4">
          <div className="flex flex-col">
            <h2 className="text-4xl pro-heading tracking-tighter">
              {type === "housing" ? "Establish" : "List"}{" "}
              <span className="text-kjc-accent italic">
                {type === "housing" ? "Post" : "Item"}
              </span>
            </h2>

            <div className="flex gap-2.5 mt-3">
              <span className="pro-badge bg-white/5 text-white/40 border-white/5 text-[8px] tracking-[0.3em]">
                EXCLUSIVELY KJC
              </span>
          ...
          <button
            type="button"
            onClick={onClose}
            className="w-14 h-14 bg-white/5 rounded-3xl flex items-center justify-center text-white/20 hover:text-white transition-all active:scale-90 border border-white/5"
          >
            <X size={24} />
          </button>
        </div>
        ...
        <div className="bg-white/5 p-8 rounded-[40px] mb-10 border border-white/5 relative overflow-hidden group">
          ...
          <div className="relative z-10 flex items-start gap-6">
            ...
            <div>
              ...
              <p className="text-[11px] text-white/30 font-bold leading-relaxed mt-1 uppercase tracking-wider">
                Your listing is anchored to your verified university profile.
              </p>
            </div>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-10 pb-4">
          ...
          <div className="space-y-4">
            ...
            <textarea
              ...
              placeholder={
                type === "housing"
                  ? "Amenities, rules, specific location vibes..."
                  : "Reason for sale, item condition details..."
              }
              rows={4}
          ...
          <div className="grid grid-cols-2 gap-6">
            <div className="space-y-4">
              ...
              <div className="relative">
                <select
                  ...
                  onChange={(e) => {
                    if (type === "housing") {
                      updateField(
                        "roomType",
                        e.target.value as ListingFormData["roomType"]
                      );
                    } else {
                      updateField(
                        "category",
                ...
                <ChevronRight
                  size={16}
                  className="absolute right-6 top-1/2 -translate-y-1/2 text-white/30 rotate-90"
                />
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="relative">
                <IndianRupee
                  size={16}
                  className="absolute left-6 top-1/2 -translate-y-1/2 text-kjc-accent font-black"
                />
